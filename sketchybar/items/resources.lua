...
local function cpu_update()
	SBAR.exec("ps -A -o %cpu | awk '{s+=$1} END {print s}'", function(total_load)
		...
		cpu:set({
			icon = { color = color or COLORS.icon.cpu },
...
local function memory_update()
	SBAR.exec("memory_pressure | grep 'System-wide memory free percentage:' | awk '{print 100-$5}'", function(result)
		...
		local color = (used > 80 and 0xffff4444) or (used > 60 and 0xffffa500) or nil
		memory:set({
			icon = { color = color or COLORS.icon.memory },
			label = { string = math.floor(used) .. "%", color = color or DEFAULT_ITEM.label.color },
		})
	end)
end
...
local popup_width = 75 -- Fixed width to prevent jitter when numbers change
local position = "left"
local prev_bytes_in = nil
local prev_bytes_out = nil
...
local network_up = SBAR.add("item", "network_up", {
	...
	icon = {
		font = { size = DEFAULT_ITEM.icon.font.size * 1.0, style = "Bold"  },
		string = "↑",
		color = COLORS.icon.network_tx,
		highlight_color = COLORS.icon.network_tx,
		padding_right = pad_r,
	},
})
...
local network_down = SBAR.add("item", "network_down", {
	...
	icon = {
		font = { size = DEFAULT_ITEM.icon.font.size * 1.0, style = "Bold"  },
		string = "↓",
		color = COLORS.icon.network_rx,
		highlight_color = COLORS.icon.network_rx,
...
local function network_update()
	-- Use netstat -ib (built-in) to read byte counters, then compute delta
	SBAR.exec("netstat -ib -I " .. interface .. " | tail -1 | awk '{print $7, $10}'", function(result)
		local bytes_in, bytes_out = result:match("(%d+)%s+(%d+)")
		bytes_in = tonumber(bytes_in) or 0
		bytes_out = tonumber(bytes_out) or 0

		local down = 0
		local up = 0

		if prev_bytes_in and prev_bytes_in > 0 then
			-- Convert bytes delta to kbps (update_freq is 2s, *8 for bits, /1000 for kbps)
			down = math.max(0, (bytes_in - prev_bytes_in) * 8 / 2000)
			up = math.max(0, (bytes_out - prev_bytes_out) * 8 / 2000)
		end
